...
void BonusFunc(user obj,shoe *sh,int n){
    int *temparr;
    temparr=obj.getID();
    system("cls");
    cout<<" -> User : "<<obj.getName()<<"\n -> ID :  {"<<temparr[0]<<" , "<<temparr[1]<<"}\nGender : ";
    if(obj.getGen()=='M')cout<<"Male";
    else cout<<"Female";
    cout<<"\nAge : "<<obj.getAge()<<"\nHeight : "<<obj.getHeight()<<"\nFoot size : "<<obj.getSize()<<endl; 

    cout<<"Whereas the information of the shoe is";
    
    for(int i=0;i<n;i++){
        cout<<"\n--Shoe "<<i+1<<endl;
    cout<<"Brand : "<<sh[i].getBrand()<<"\nColor : "<<sh[i].getColor()<<"\nSize : "<<sh[i].getSize()<<"\nWidth : "<<sh[i].getWidth()<<"\nStyle : ";
    

    switch(sh[i].getStyle()){
        case 0 :cout<<"Running Shoes" ;break;
        case 1 :cout<<"Sneakers"; break;
        case 2 :cout<<"Stiletto"; break;
    } 
